    ...
    return (
        <aside className="filter-sidebar">
            ...
            <form>
                ...
                <input style={{ accentColor: "green" }} onChange={(e) => dispatch({ type: "SORT_BY_PRICE_RANGE", payload: e.target.value })} value={price} type="range" min="200" max="1000" className="slider" />
            </form>
            ...
            <form>
                {categories.map(({ _id, categoryName }) =>
                    <div key={_id}>
                        <input style={{ accentColor: "green" }} onChange={() => dispatch({ type: "SORT_BY_CATEGORY", payload: categoryName })} checked={category.includes(categoryName)} id={categoryName} type="checkbox" />
            ...
            <form>
                {ratingNumbers.map((num) =>
                    <div key={num}>
                        <input style={{ accentColor: "green" }} onChange={() => dispatch({ type: "SORT_BY_RATING", payload: num })} checked={rating === num ? true : false} id={`${num}stars`} type="radio" name="rating" />
            ...
            <form>
                <input style={{ accentColor: "green" }} onChange={() => handleSort("LOW_TO_HIGH")} checked={sortBy === "LOW_TO_HIGH" ? true : false} id="low-to-high" type="radio" name="sort-by" />
                <label htmlFor="low-to-high">Price - Low to High</label><br />
                <input style={{ accentColor: "green" }} onChange={() => handleSort("HIGH_TO_LOW")} checked={sortBy === "HIGH_TO_LOW" ? true : false} id="high-to-low" type="radio" name="sort-by" />
